...
public class SongH2Service implements SongRepository {
    ...
    public Song addSong(Song song) {
        db.update("insert playlist into(songName,lyricist,singer,musicDirector) values(?,?,?,?)", song.getSongName(),song.getLyricist(), song.getSinger(), song.getMusicDirector());
        Song savedSong = db.queryForObject("select * from playlist whre songName = ?, lyricist = ?, singer = ? and musicDirector = ?",new SongRowMapper(), song.getSongName(), song.getLyricist(), song.getSinger(), song.getMusicDirector());
        return savedSong;
    }
